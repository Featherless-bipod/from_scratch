...
def slstm_forward_pointwise(
    ...
    # Avoid CPU sync with .item() by using torch.where directly on tensors
    n_is_zero = (n == 0.0)
    mnew = torch.where(n_is_zero, iraw, torch.max(iraw, logfplusm))
    ...
    new_states = torch.stack((ynew, cnew, nnew, mnew), dim=0)
    gates = torch.stack((igate, fgate, zraw, ogate), dim=0)
...
class sLSTMCell(nn.Module):
    ...
    def forward(self, x, states, constants=None):
        ...
        new_states, gates = slstm_forward_pointwise(wx, ry, self.b, states)
        return new_states, gates
